...
# Minimal interpreter for .prot scripts.
# Godot 4.x
...
func run_source(source_text: String) -> void:
	...
	for i in range(lines.size()):
		...
		var op := String(tokens[0]).to_upper()
...
'''func _exec_prnt(tokens: Array, line_no: int) -> void:
	...
	# If the first argument is a quoted string token, print it directly.
	# Otherwise treat everything after PRNT as an expression.
	if _is_quoted(tokens[1]) and tokens.size() == 2:
		output.call(_unquote(tokens[1]))
		return

	var expr_tokens := tokens.slice(1)
	var value = _eval_expression(expr_tokens, line_no)
	output.call(str(value))'''
...
'''func _exec_var(tokens: Array, line_no: int) -> void:
	...
	# VAR Y = 1 + 2
	if tokens.size() < 2:
		_runtime_error(line_no, "VAR requires a variable name")
		return

	var name := String(tokens[1])
	if not _is_valid_var_name(name):
		_runtime_error(line_no, "Invalid variable name: %s" % name)
		return
	...
	# Allow optional '='
	var rhs := tokens.slice(2)
	if rhs.size() > 0 and rhs[0] == "=":
		rhs = rhs.slice(1)

	# If it's a single quoted string -> store as string
	if rhs.size() == 1 and _is_quoted(rhs[0]):
		vars[name] = _unquote(rhs[0])
		return

	vars[name] = _eval_expression(rhs, line_no)'''
...
func _exec_math_cmd(opcode: String, tokens: Array, line_no: int) -> void:
	...
	if not _is_valid_var_name(dest):
		_runtime_error(line_no, "Invalid variable name: %s" % dest)
		return
	...
	if tokens.size() == 2:
		a = _get_var_numeric(dest, line_no)
		b = a
	elif tokens.size() == 3:
		a = _get_var_numeric(dest, line_no)
		b = int(tokens[2])
	...
	match opcode:
		...
		"DIV":
			if float(b) == 0.0:
				_runtime_error(line_no, "Division by zero")
			else:
				vars[dest] = a / b
...
func _tokenize(line: String) -> Array:
	# Splits on whitespace, but keeps quoted strings as one token.
	var tokens: Array = []
	var i := 0
	while i < line.length():
...
func _get_var_numeric(name: String, line_no: int) -> float:
	if not vars.has(name):
		_runtime_error(line_no, "Variable %s does not exist" % name)
		return 0.0
	var v = vars[name]
	if typeof(v) == TYPE_INT or typeof(v) == TYPE_FLOAT:
		return float(v)
	if typeof(v) == TYPE_STRING and _is_number(String(v)):
		return float(v)
	_runtime_error(line_no, "Variable %s is not numeric" % name)
	return 0.0
